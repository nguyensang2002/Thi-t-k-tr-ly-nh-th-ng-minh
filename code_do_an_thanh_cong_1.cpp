...
const unsigned long sendInterval = 5000; // Thời gian giữa các lần gửi dữ liệu (đơn vị: milliseconds)
...
void sendSensor() {
  ...
  if (currentTime - lastSendTime >= sendInterval) { // Kiểm tra xem đã đến lúc gửi dữ liệu chưa
    readSensor();
...
void setup() {
  ...
  xTaskCreate(TaskBlynk, "TaskBlynk", 10000, NULL, 1, &TaskBlynkHandle);
  xTaskCreate(TaskOled, "TaskOled", 10000, NULL, 2, &TaskOledHandle);
  xTaskCreate(TaskButton, "TaskButton", 10000, NULL, 3, &TaskButtonHandle);
  xTaskCreate(TaskSensor, "TaskSensor", 10000, NULL, 4, &TaskSensorHandle);
...
void listen_push_buttons() {
  ...
  const unsigned long debounceDelay = 500; // Thời gian chờ debounce
  unsigned long currentMillis = millis();

  if (currentMillis - lastDebounceTime < debounceDelay) {
    return; // Nếu chưa đủ thời gian debounce, không làm gì cả
  }
...
void TaskSensor(void *pvParameters) {
  (void) pvParameters;
  while (1) {
    readSensor(); // Đọc dữ liệu từ cảm biến BME280
    sendSensor(); // Gửi dữ liệu lên Blynk Cloud
    vTaskDelay(1100 / portTICK_PERIOD_MS); // Chờ 1,1 giây (1100ms)
  }
}
